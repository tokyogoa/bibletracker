document.addEventListener('DOMContentLoaded', () => {
    ...
    const bibleData = { ...bibleStructure["구약"], ...bibleStructure["신약"] };
    ...
    function renderDashboard() {
        ...
        if (users.length === 0) {
            dashboard.innerHTML = '<div class="empty-message">아직 등록된 멤버가 없습니다.<br>첫 번째 멤버를 추가해보세요!</div>';
            return;
        }
    ...
    function populateFilter() {
        viewFilter.innerHTML = '<option value="all">전체 진행 상황</option>';

        for (const category in bibleStructure) {
            const optgroup = document.createElement('optgroup');
            optgroup.label = category; // "구약" or "신약"
            for (const book in bibleStructure[category]) {
                const option = document.createElement('option');
                option.value = book;
                option.textContent = book;
                optgroup.appendChild(option);
            }
            viewFilter.appendChild(optgroup);
        }
    }
